...
namespace KifuFilter
{
    class TesuuSplitter
    {
        ...
        /// </summary>
        /// <param name="input">入力ファイル名またはディレクトリ名</param>
        ...
        public static void filter(string input, string outputFileName, int splitWidth, int maxPly)
        {
            string[] path = new[] { input };

            if (Directory.Exists(input))
            { //ディレクトリならその中のすべてのファイル
                path = Directory.GetFiles(input);
            }
            ...
            foreach (string inputFileName in path)
            {
